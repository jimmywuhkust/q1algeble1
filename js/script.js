...
function flick() {
  ...
  if (show == false) {
    ...
    document.getElementById("c10").textContent = ""
    show = false
  }
}
...
function handleMouseClick(e) {
  ...
  if (e.target.matches("[data-key]")) {
    if (guessans && "p-t/".search(e.target.dataset.key)!=-1) {return;}
  ...
  if (e.target.matches("[data-guessb]")) {
    if (guessans == false) {
      ...
      guessGridb.querySelector('[data-guess="guess"]').remove();
      guessGridb.querySelector('[data-guess="guess"]').remove();
      guessGridb.innerHTML += "<div class=\"tile\" data-guess=\"guess\">?</div>\
      <div class=\"tile\" data-guess=\"guess\">?</div><div class=\"tile\" data-guess=\"guess\">?</div>"
...
function pressKey(key) {
  ...
  if (guessans) {
    if (guessGridb.querySelectorAll('[data-state="active"]').length >= 3) return
  ...
  if (activeTiles.length==WORD_LENGTH-1 && error.search('1') == -1 && eval(ptchange(guess))!=reqGA) {
    let answerTiless = [...getAnswerTiles()]
    for (let i = 0; i < answerTiless.length; i++) {
...
function submitGuess() {
  if (guessans) {
    ...
    if (guessTiles.length == 3) {
      ansguess = guessTiles.reduce((word, tile) => {
        return word + tile.dataset.letter
      }, "")
      if (ansguess == targetAnswer) {
        globalwin=true;
        guesscount+=1;
...
function flipTile(tile, index, array) {
  ...
  const check = checkpos(ptchangerev(guess), ptchangerev(targetWord), index)
...
function guessansf() {
  ...
  guessGridb.querySelector('[data-guess="guess"]').remove();
  guessGridb.querySelector('[data-guess="guess"]').remove();
  guessGridb.innerHTML += "<div class=\"tile\" data-guess=\"guess\" data-input=\"input\"></div>\
  <div class=\"tile\" data-guess=\"guess\" data-input=\"input\"></div><div class=\"tile\" data-guess=\"guess\" data-input=\"input\"></div>";
}
